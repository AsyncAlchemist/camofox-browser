...
import os from 'os';
import { readStoredProxy } from './proxy-store.js';
...
/**
 * Whether a bind host is an EXCLUSIVE loopback address — i.e. a server bound to
 * it is unreachable from any other machine. Only such a binding lets the OS
 * guarantee every caller is local, so it is the sole condition under which auth
 * is bypassed.
 *
 * Loopback-exclusive: 127.0.0.0/8 (e.g. 127.0.0.1), ::1, and the literal
 * "localhost". NOT loopback-exclusive: 0.0.0.0 / :: (all interfaces) or any
 * specific external/LAN IP — those are reachable off-box, so auth is required.
 */
...
function loadConfig() {
  ...
  // Bind address for the HTTP server. Default 0.0.0.0 (all interfaces) for
  // backward compatibility with Docker/Fly/Railway deployments. Set
  // CAMOFOX_HOST=127.0.0.1 for a loopback-exclusive (local-only) server.
  const host = process.env.CAMOFOX_HOST || process.env.HOST || '0.0.0.0';
  // A proxy assigned via `camofox proxy use` is read here as a fallback so a
  // locally-run server (node server.js) honors it without manual env wiring.
  // Explicit PROXY_* env always wins. (For the Docker path the CLI forwards
  // these as -e flags instead, since the container can't read ~/.camofox.)
  ...
  return {
    ...
    // Auth is bypassed ONLY when the server is bound exclusively to loopback, so
    // the OS guarantees every caller is on this machine. Any external/all-interface
    // binding (0.0.0.0, ::, a LAN/public IP) requires auth for everybody.
    // Derived from the bind host — never an operator assertion.
    trustLocal: isLoopbackHost(host),
    ...
    proxy: {
      strategy: inferProxyStrategy(process.env.PROXY_STRATEGY || storedProxy.strategy || ''),
      providerName: process.env.PROXY_PROVIDER || 'decodo',
      host: process.env.PROXY_HOST || storedProxy.host || '',
      port: process.env.PROXY_PORT || (storedProxy.port ? String(storedProxy.port) : ''),
      ports: parseProxyPorts(process.env.PROXY_PORTS, process.env.PROXY_PORT || storedProxy.port),
      username: process.env.PROXY_USERNAME || storedProxy.username || '',
      password: process.env.PROXY_PASSWORD || storedProxy.password || '',
      backconnectHost: process.env.PROXY_BACKCONNECT_HOST || '',
      backconnectPort: parseInt(process.env.PROXY_BACKCONNECT_PORT || '7000', 10),
      country: process.env.PROXY_COUNTRY || storedProxy.country || '',
